...
function switchTab(tabId) {
  // HAPUS SISA DISPLAY BLOCK (BIANG KEROK BUG NUMPUK)
  document.querySelectorAll('.container').forEach(c => {
      c.classList.remove('active');
      c.style.display = ''; // Bersihin inline style
  });
  ...
  // PENGATURAN VISIBILITAS ELEMEN ATAS & BAWAH
  ...
  // LOGIKA FAB "SIMPAN KOTOBA" (Cuma muncul di Preview, Home, dan Flashcard)
  let fab = document.getElementById('fab-saved');
  if(fab) {
      if(tabId === 'home' || tabId === 'preview' || tabId === 'flashcard-app') {
...
async function loadData() {
  try {
      ...
      // Sempurnakan transisi agar tidak numpuk
      switchTab('home');
